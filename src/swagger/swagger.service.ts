...
export class SwaggerService {
  ...
  async getSwagger(repoPath): Promise<string> {
    ...
    const endpoints = [];
    endpointNamesPath.forEach(folderPath => {
      const endpointNameWithoutPath = folderPath.replace(
        cache + repoPath + '/',
        '',
      );
      const schema = this.schemaService.getSchema(folderPath);
      if (schema) {
        endpoints.push({ [endpointNameWithoutPath]: schema });
      }
    });
